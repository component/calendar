...
Days.prototype.rowsFor = function(date){
  ...
  for (var i = 0; i < total; ++i) {
    ...
    if (day == selectedDay && month == selectedMonth && year == selectedYear) {
      cells.push('<td class=selected><a href="#" data-date=' + year + '-' + month + '>' + day + '</a></td>');
    } else {
      cells.push('<td><a href="#" data-date=' + year + '-' + month + '>' + day + '</a></td>');
    }
  }
...
function prevMonthDay(year, prev, day) {
  return '<td><a href="#" data-date=' + year + '-' + prev + ' class=prev-day>' + day + '</a></td>';
}
...
function nextMonthDay(year, next, day) {
  return '<td><a href="#" data-date=' + year + '-' + next + ' class=next-day>' + day + '</a></td>';
}
